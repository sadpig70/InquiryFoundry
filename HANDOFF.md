...
| 7.46 ifa 예측 포트폴리오 | 현행 | §7.22 첫 소비자. 파일럿 register 11/discard 7 |
| 7.47 ifa 전 포트폴리오 완주 | 현행 | 90/90 예측. **split 5.** 71건 single(LWAR1 블라인드 전멸) |
| 7.48 신규 3사 자격 패스 | 현행 | Kimi 75/Qwen 62.5/GLM 60.4%. **Q-0041 = 7:8 split.** 잔여 63 |
| 7.49 DeepSeek 100% · 캘리브레이션 의문 | **기록** | §7.50 에서 분리 완료 |
| 7.50 포트폴리오 완성 · 분리 성공 | 현행 | **드리프트 +24.6pt 실재, DeepSeek 우위도 실재.** split 30 |
...
## 7.46 ifa — 채택 질문에 대한 예측 포트폴리오, 첫 소비자 (2026-08-27)

운영자 지시로 PGF full-cycle(설계·검토·구현·테스트·검증)을 한 번에 진행했다.
권위 설계 `.pgf/DESIGN-IfaCore.md`(결정 A1–A13), 소스 `.agents/skills/ifa-core/`,
저장소 `.ifa/`(git 제외). **§7.22(채택물의 소비자 부재)의 첫 소비자다.**

### 무엇인가

IF 가 채택한 질문에 대해 같은 방법론(감옥·이종 벤더·교차 반박·블라인드 채점·출처
가린 리뷰·기계 종결 불가)으로 **사전 등록 예측**을 생산한다. 산출물은 대답이 아니다 —
`kind` enum 이 `["predicted"]` 단일값이고, `answered` 는 **스키마에 존재하지 않는다**(A3).
실제 실험이 수행되는 날 그 값의 추가가 명시적 스키마 변경이 되도록.

격리(A1): `.if` 에 쓰기 0바이트 — `assert_outside_if` 가드 + 테스트. AnswerObject 가
`question_id` 를 드는 단방향 참조. `if_core` 는 역할 무관 유틸만 import(원본 무수정).
bus 는 적응 복사(A12) — `make_pao_task` 가 IF 역할에 결합돼 있어서이고, live10g 의
stale 매칭 교훈을 이식했다. **IF 런과 동시 실행 금지**(A6, 미검증 운영 규칙).

### 파일럿 — RUN-IFA-20260827-p1, 전 구간 실 LWAR

- 배치: 정량 reject_if 기준 6건 (scaling 3 + preference 3)
- predict(3벤더 독립) → rebut(회전) → adjudicate(블라인드): **9/9 succeeded, drop 0**
- AnswerObject 18건 → LWAR3 리뷰 → 위임 비준 → **register 11 / discard 7**
- `.if` 무변경 실측 확인. 테스트 105 (ifa 8 + if 97)

**리뷰가 실질적이었다** — discard 사유가 숨은 전제(*"부호가 '참조 정책의 greedy 길이가
더 짧다'는 전제에 달려 있는데 근거는 그 순서를 재지 않는다"*)와 근거 오귀속
(*"hoffmann2022 가 말하지 않은 것을 말한 것처럼 썼다"*)을 짚었다. 문헌 요약이 아니라
검증 가능한 추론의 선별이 일어났다.

### 첫 우선순위 보고서가 말하는 것

`.ifa/reports/priorities-RUN-IFA-20260827-p1.md`:

| 질문 | 등록 예측 | 불일치 |
|---|---|---|
| 5건 | 각 2 | **agree** (전부 no-reject 합의) |
| 1건 | 1 | single |

이번 배치에 split 이 없다 — 등록된 예측들이 방향에서 합의한다. 정직한 읽기:
**이 5건은 실험 정보 이득이 낮은 쪽**이라는 신호이고(모델들이 이미 답을 안다고
믿는다), split 이 나오는 질문을 찾는 것이 이 시스템의 존재 이유다. 6건 표본에서
split 0 은 배치 확대의 근거이지 실패가 아니다 — 단, 그 판단은 별도 지시 사안.

### 남긴 것

- 되먹임 루프 없음(A13) — 필요가 실측되면 그때
- `disagreement` 의 3단계(diverge)는 방향이 이진이라 지금은 2단계(agree/split) —
  스칼라 예측 도입 시 확장

## 7.47 ifa 전 포트폴리오 완주 — 그리고 블라인드가 확인한 것 (2026-08-27)

정량 reject_if 를 가진 채택 질문 **90건 전부**가 예측을 받았다. 11개 런(파일럿 포함),
드라이버 자동 진행. 예측 232건 → **등록 106 / 기각 126**. 집계:
`.ifa/reports/priorities-ALL.md`.

### 산출물 — 실험 우선순위

| 불일치 | 질문 수 | 뜻 |
|---|---:|---|
| **split** | **5** | 잘 근거된 예측이 방향에서 갈림 — **실험 정보 이득 최상위** |
| agree | 14 | 등록 예측들이 합의 — 정보 이득 낮음 |
| single | 71 | 등록 예측 1건뿐 — 불일치 계산 불가 (아래) |

split 5건: Q-20260821-0004 · 0010 · Q-20260822-0037 · 0038 · 0039
(0038 은 live14 의 '승자 접미사 오염' — DUP 사건에 여러 번 등장한 그 질문).

### 운영 사고 2건 (해결됨)

- **run_id 재사용 충돌** — 드라이버 v2 가 번호를 다시 세서 b01 을 재사용, 옛 jail 의
  stale outbox 가 새 배치 검증에 걸려 라운드 실패. 답변 저장소는 무손상(저장 전 실패).
  v3 가 기존 런 번호에서 이어 세고, b01 report 는 답변 저장소에서 재구성.
- **LWAR2(Grok) 이탈** — b03 에서 파킹 이탈, 복귀 없음. v2 부터 2벤더 하한으로 속행.

### 블라인드가 확인한 것 — 71건이 single 인 이유

b04 이후 리뷰어는 **출처를 못 보는 상태에서** LWAR1(Antigravity)의 예측 **68건 연속
discard**, LWAR4(Codex)만 등록했다. 사유는 실질적이다 — 근거 과장("casper2023 이
'체계적으로 규명'했다는 귀속이 근거를 넘는다"), 미검증 가정, rationale 과 prediction 의
부호 모순. **품질 격차가 블라인드를 뚫고 구분된다** — live22~24 에서 LWAR1 의 질문이
3런 연속 0/9 기각된 것과 같은 런타임, 같은 방향의 신호다. 편향 해석은 기각된다:
리뷰어는 저자를 알 수 없었다.

### 한계와 자연 후속 (미착수)

71건의 single 은 대부분 LWAR4 단독 등록이라 **그 질문들의 불일치 정보가 없다.**
두 번째 의견 패스(등록 1건 질문만 골라, 복귀한 LWAR2 나 갱신된 LWAR1 세션으로 재예측)
가 자연 후속이다 — select 가 예측 완료 질문을 제외하므로 별도 선별 모드가 필요하다.
착수하지 않았다.

## 7.48 신규 3사 자격 패스 — 성적표와 뜻밖의 고밀도 표본 (2026-08-30)

GLM 5.3(zai) · Qwen(alibaba) · Kimi 2.7(moonshot) 가 LWAR2/5/6 으로 등록했고
(구 LWAR2=Grok 은 크레딧 소진으로 은퇴), second-opinion 자격 패스를 돌렸다.
7배치 완주(답변 112건), Fable 블라인드 리뷰로 register 70 / discard 42.

### 자격 성적표 (블라인드 등록률 — 리뷰어는 저자를 못 본다)

| 벤더 | 표본 | 등록률 |
|---|---:|---:|
| Codex (기준선) | 90 | **98.9%** |
| Grok (기준선, 은퇴) | 22 | 90.9% |
| **Kimi 2.7** | 8 | 75.0% (표본 小 — 지연 합류 1배치뿐) |
| **Qwen** | 56 | 62.5% |
| **GLM 5.3** | 48 | 60.4% |
| Antigravity (기준선) | 90 | 1.1% |

**주의 — `vendors` 성적표의 슬롯 번호 함정**: LWAR2 가 구 Grok 과 신 GLM 을 합산한다
(슬롯 재사용). 위 표는 런 집합(구/자격)으로 분리해 교정한 값이다. 도구는 slot id 로
집계하므로 **세대를 건널 때는 런 집합 분리가 필수**다.

### 내 버그 — 그리고 그것이 만든 고밀도 표본

second-opinion 선별이 미리뷰(DRAFT/SCORED) 진행분을 제외하지 않아, 리뷰를 뒤로 미룬
2단계 구조에서 **7배치가 같은 8질문을 반복 예측**했다. 수정했고(진행분 제외 + 테스트),
`minority_frac` 지표도 추가했다(14:1 과 8:7 을 같은 split 으로 부르던 조잡함 보완).

버그의 소득: 그 8질문에 예측이 14~15건씩 쌓여 **고밀도 분포**가 생겼다.
**Q-20260822-0041 은 15예측이 7:8** — 현 포트폴리오에서 실험 정보 이득 최상위.
반면 Q-0004(14:1)·Q-0040(8:1)은 사실상 합의로 판명.

### 세션 지속성 (운영 비용 축)

- GLM·Qwen: **7배치 연속 무이탈** 후 소진성 이탈 — 구세대보다 훨씬 낫다
- Qwen: 반박/채점 단계 outbox 무효 2회 (q03·q07 — 품질과 별개의 형식 규율 문제)
- Kimi: 지시 오해로 지연 합류(§ 운영자 개입 1회), 합류 후엔 정상

### 잔여 (정지 시점)

second-opinion 63건 남음. 후보 2사 이상 파킹이 재개되면 이어서 걷는다 —
select 가 진행분을 제외하므로 그대로 재기동하면 된다.

## 7.49 DeepSeek 데뷔 만점 — 그리고 그 숫자를 그대로 믿지 않는 이유 (2026-09-01)

DeepSeek(opencode 런타임, **LWAR8**)이 등록→파킹을 무개입 완주했고, Kimi 신규 세션
(**LWAR7**)과 함께 자격 배치 4개(q10~q13, 32질문)를 걸었다. Fable 블라인드 리뷰 종결.

### 세대 교정 성적표

| 벤더 | 세대 | 표본 | 블라인드 등록률 |
|---|---|---:|---:|
| **DeepSeek** | q2 | 32 | **100.0%** |
| Codex (기준선) | old | 90 | 98.9% |
| Grok (은퇴) | old | 22 | 90.9% |
| **Kimi 2.7** | q2 | 32 | **81.2%** |
| Qwen | q1 | 56 | 62.5% |
| GLM 5.3 | q1 | 48 | 60.4% |
| Antigravity | old | 90 | 1.1% |

### 100% 를 그대로 믿지 않는 이유 — 적어 둔다

q1 배치들(GLM·Qwen, 08-30 리뷰)의 배치당 등록률은 ~60%, q2 배치들(Kimi·DeepSeek,
09-01 리뷰)은 ~91% 다. 두 읽기가 갈린다: (a) 두 벤더가 실제로 더 강하다,
(b) **리뷰어 캘리브레이션이 날짜를 건너 이동했다.** 같은 리뷰어가 다른 날 다른
벤더군을 리뷰했으므로 이 런들로는 분리되지 않는다 — live22 로스터/인프라 교락과
같은 형태다. 분리 방법: 잔여 31건을 **q1 벤더와 q2 벤더 혼합 배치**로 걸면
같은 날·같은 리뷰 안에서 직접 비교가 된다. GLM/Qwen 세션이 복귀하면 그렇게 한다.

### 불일치 지도 (누적 90질문 중 59건 리뷰 완료 기준)

split 19 · agree 40 · single 31. **깊은 split(minority≥30%) 16건** —
최상위는 여전히 Q-20260822-0041(n=15, 40%). 실험 후보 명단이 두 자릿수가 됐다.

### 운영 기록

- Kimi: 스스로 재파킹해 4배치 무이탈 후 q14 직전 이탈 — 세션 duty cycle 패턴 지속
- 구 LWAR6(Kimi 이전 세션) 잔여 — 은퇴 대기
- 잔여 second-opinion 31건 — 후보 2사 파킹 시 자동 재개(체인 재기동 필요)

## 7.50 예측 포트폴리오 완성 — 캘리브레이션과 실력이 분리됐다 (2026-09-01)

Qwen 이 RESUME 경로(무개입, gen 유지)로 복귀해 DeepSeek 과 **혼합 배치** 4개를 걸었고,
second-opinion 풀이 **소진**됐다. **90질문 전부가 복수 벤더의 사전 등록 예측을 가진다.**

### 혼합 배치가 §7.49 의 격리를 풀었다 — 둘 다 사실이었다

| 벤더 | wave1 (08-30 리뷰) | mixed (09-01, 같은 런·같은 리뷰) |
|---|---:|---:|
| Qwen | 62.5% | **87.1%** |
| DeepSeek | — | **96.8%** (wave2 100%와 정합) |

- **캘리브레이션 이동은 실재한다**: 같은 Qwen 이 리뷰 날짜만 바뀌어 +24.6pt.
  wave 간 성적 비교는 무효 — Kimi 81.2%(wave2)도 같은 폭으로 할인해 읽어야 한다.
- **DeepSeek 의 우위도 실재한다**: 같은 날·같은 런 안에서 96.8% vs 87.1% —
  약 10pt 의 실력 격차가 캘리브레이션과 무관하게 성립한다.
- Codex 98.9% 와의 비교는 여전히 세대 교락(구 시대 리뷰) — 서열 확정은
  같은 런 동거가 필요하다.

### 최종 산출물 — 90질문 불일치 지도 (`priorities-ALL.md`)

**split 30 / agree 60, 깊은 split(minority≥30%) 27건.** single 0 —
모든 질문이 불일치 판정 가능 상태가 됐다. 최상위는 Q-20260822-0041(15예측 7:8).
**이것이 "무엇을 실제로 실험할 것인가" 에 대한 이 시스템의 완성된 첫 답이다.**

### 로스터 사건 기록

- Kimi(LWAR7): 5시간 리밋으로 사망 — **q13 rebut claim 을 쥔 채**. 그 런은 이미
  drop 종결이라 무손실. `delivery-timeout` + `--abandoned-task-id` 가드로 은퇴.
  구세션 LWAR6 도 은퇴. registry v68.
- Qwen RESUME: 재등록 없이 identity 로 복귀 — 경로 A 가 실전 검증됐다.
- 현 로스터: LWAR1(교체 대상)·LWAR2(GLM)·LWAR3(리뷰어)·LWAR4(Codex)·LWAR5(Qwen)·LWAR8(DeepSeek).

### 다음 판단 재료 (안건, 미착수)

- 로스터 재편: DeepSeek 은 데이터상 즉시 주전감. Antigravity 교체 확정적.
  IF 생성 3대 구성(예: Codex+DeepSeek+Qwen)과 검증 4순위 후보들은 운영자 결정.
- 서열 확정용 같은-런 동거 배치(Codex 포함)는 필요 시 1런이면 된다.

## 7.51 튜닝된 Grok 의 재자격 — 가장 어려운 시험지에서 최고점 (2026-09-02)

운영자가 model·think mode 를 조정한 Grok 을 재가동했고, **contested 30건**(등록 예측이
갈린 쟁점 질문 — 시험지 중 가장 어려운 세트)으로 재자격을 치렀다. 등록→승인(LWAR6,
xai)→파킹→시험까지 전 과정 자동. 4배치, 같은 런·같은 리뷰의 3사 비교:

| 벤더 | 시험 성적 (q20~q23) | 참고 |
|---|---:|---|
| **Grok(tuned)** | **29/30 = 96.7%** | 구세대 90.9% 에서 상승 — 튜닝 효과로 읽히나 시험지가 달라 직접 비교는 아님 |
| Qwen | 14/16 = 87.5% | q22 부터 이탈(2배치만) |
| DeepSeek | 26/30 = 86.7% | second-opinion 96.8% → **쟁점 세트에서 10pt 하락** |

**쟁점 세트는 점수를 깎는다** — DeepSeek 의 하락이 그 증거다(진짜 불확실한 질문에서는
잘 근거된 예측이 어렵다). 그 세트에서 96.7% 를 낸 튜닝 Grok 은 **동일 조건 비교에서
현행 1위**다. 주의: 각 벤더의 헤드라인 숫자는 시험지가 달라 서로 직접 비교 불가 —
유효한 것은 **같은 런 안의 상대 비교**뿐이고, 그 비교에서 Grok > Qwen ≈ DeepSeek.

**슬롯 재사용 함정 재발**: LWAR6 이 구 Kimi 세션과 재사용 충돌 — vendors 성적표의
LWAR6 행은 두 벤더 합산이다. 위 표는 런 집합 분리 교정값(교정 절차는 §7.48 과 동일).

### 동점 해소 효과 — 쟁점 30건에 캐스팅 보트가 들어갔다

Q-0041 은 18예측 minority 33%(7:8 → 12:6 방향), 깊은 split 은 27→**12건**으로 압축 —
추가 의견이 다수를 확정지어 지도가 날카로워졌다. split 자체는 30 유지(경계 재계산).

### 로스터 데이터 종합 (동일-런 비교 기준)

Grok(tuned) 96.7 > Qwen 87.5 ≈ DeepSeek 86.7 (쟁점 세트) ·
DeepSeek 96.8 > Qwen 87.1 (2nd-opinion 세트) · Codex 98.9 (구세대, 미교차).
**Codex 포함 동거 1런이 서열 확정의 마지막 조각**이다. 생성 3대 구성은 운영자 결정.

## 7.52 전원 은퇴, 버스 클리어 — 그리고 가드 교착 발견 (2026-09-02)

운영자 지시로 7슬롯 전부 은퇴, registry **v76, 슬롯 0**. 활성 큐 전량 0, 요청 큐 0.
pao-server 는 유지(운영자 방침 전례). 채택 113 · 예측 포트폴리오(90질문 지도)는
`.if`/`.ifa` 에 온전하다.

### 발견 — 공식 도구의 가드 교착 (미수정)

**"작업 중 사망 + control 대기" 조합은 공식 명령으로 풀 수 없다**:

- `recover --expire-controls` 는 heartbeat `status=running` 이면 **나이 무관 무조건
  거부**(exit-notify 설계상 옳다 — 작업 중 heartbeat 는 동결되므로 나이로 생사를
  못 가른다) — 그러나 `--abandoned-task-id` 같은 탈출구가 **없다**.
- `recover --retire-stale --abandoned-task-id` 는 heartbeat 검사는 통과하나
  **mailbox 검사(control 잔여)** 에서 거부.
- 두 가드가 서로의 전제를 잠근다. LWAR2·5 (며칠 전 adjudicate 중 사망 + 이번
  shutdown 미수령)가 정확히 이 상태였다.

**이번 처리(문서화된 이탈)**: 잔여 control 2건이 (a) 내가 같은 시간에 발행한
shutdown 이고 (b) 내용 검증 후 (c) 수신자 사망이 확정된 상태에서, 운영자의 명시적
전체 클리어 지시 하에 격리 보관(`_workspace/orphan-debris-20260902/`) 후 공식
은퇴를 완료했다. 파일 삭제가 아니라 이동이며, 이 문단이 그 기록이다.

**향후 수정 안건**: `expire-controls` 에 `--abandoned-task-id` 탈출구를 추가하면
교착이 공식 경로로 풀린다. pao_runtime 수정이므로 별도 승인 사안.

### 재가동 시 참고

전 슬롯이 비었으므로 다음 세션들은 전부 신규 등록(LWAR1 부터 배정)이다.
로스터 구성 판단 재료는 §7.51 종합 참조 — 동일런 비교: Grok(tuned) 96.7 >
Qwen 87.5 ≈ DeepSeek 86.7 (쟁점) · DeepSeek 96.8 (커버리지) · Codex 98.9 (구세대).

## 7.53 최상위 3사 동거 시험 — 서열이 뒤집혔고, 그것이 결론이다 (2026-09-02)

Codex·Grok(tuned)·DeepSeek + Fable(리뷰어) 4세션이 빈 판에서 등록했고(LWAR1=xai,
2=deepseek, 3=anthropic, 4=openai), 쟁점 30건을 **3사 동시**로 걸었다. 4배치 drop 0,
전 과정(승인→ping→파킹→시험→리뷰) 무개입 자동.

### 동일 런 성적 (q25~q28) — 그리고 어제(q20~q23)와의 충돌

| 벤더 | 오늘 (3사 동거) | 어제 (쟁점 세트) |
|---|---:|---:|
| **DeepSeek** | **96.7%** | 86.7% |
| **Codex** | **93.3%** | — |
| **Grok(tuned)** | **80.0%** | 96.7% |

**Grok 과 DeepSeek 의 서열이 하루 만에 뒤집혔다.** 각 런 안의 비교는 유효하므로
이 충돌 자체가 발견이다: **같은 벤더의 세션 간 변동이 벤더 간 격차와 같은 자릿수다.**
(시험지 부분집합 차이도 겹친다 — 동점 해소로 쟁점 풀이 이동했다.)

**흔들리지 않은 것은 Codex 다**: 구세대 98.9%(90건) + 오늘 93.3%(가장 어려운 세트)
— 어느 시대·어느 시험지에서도 93 밑으로 내려간 적이 없는 유일한 벤더.
합산 120건 97.5%.

### 로스터 판단 재료 (최종)

- **일관성 서열: Codex > DeepSeek > Grok(tuned)** — 절대치가 아니라 분산 기준.
  셋 다 합격선 위이고, Antigravity(1.1%)류의 탈락자는 없다.
- 단일 시험으로 서열을 확정하려는 시도는 이번 충돌로 반증됐다 — **세션 변동이
  지배하는 구간에서는 순위가 아니라 등급(합격/탈락)이 유효한 결론이다.**
