...
## 7.19 pattern 을 코드로 — Fable 5 결정 (2026-08-22, **비준 대기**)
...
## 7.22 산출물 정리 — 채택 질문 58건 (2026-08-22)
...
**레지스트리는 양 도메인 모두 비어 있다.** `avoid_codes.yaml` 이 `ratified: true` 라
`patterns` 블록이 택소노미에서 나오고, 레지스트리의 입장(2런 재발)·퇴장(4런 미사용)·
상한 기계는 한 건도 넣은 적이 없다. 현재 사실상 미사용 경로다 — 지우자는 것이 아니라,
살아 있다고 가정하고 판단하지 말라는 뜻이다.
