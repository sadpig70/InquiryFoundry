...
from .adp_watch import run_exit_notify, run_live_notify, validate_watch_args, watch
from .common import (
    atomic_write_json,
    emit,
    file_identity_leaks,
...
def _watch_namespace(identity_path: Path, root: Path, args: argparse.Namespace) -> argparse.Namespace:
    return argparse.Namespace(
        ...
        report_every=getattr(args, "report_every", 86400.0),
...
def command_response(args: argparse.Namespace) -> int:
    root = resolve_root(args.root)
    if args.resident or getattr(args, "background", False) or getattr(args, "detach", False):
    ...
    if getattr(args, "detach", False) and getattr(args, "background", False):
        from .detach import (
        ...
        events = event_log_path(root, args.request_id)
        argv = [sys.executable, *child_argv_without_detach()]
        pid = spawn_detached(argv, event_file=events)
        rec_path = pending_pid_path(root, args.request_id)
        write_pid_record(
            rec_path,
            {
                "pid": pid,
    ...
    wait_in_process = bool(args.resident or getattr(args, "background", False))
    if not response_path.is_file():
        if wait_in_process:
            # --resident and --background both wait here. Kimi's official
            # path is --resident --max-runtime-s 3000: exit so the host can
            # restart instead of dying at the 1h job cap.
            while not response_path.is_file():
                now = time.monotonic()
                if max_rt is not None and (now - wait_started) >= float(max_rt):
                    emit(
                        {
                    ...
                    return 2
                sleep_s = max(float(args.interval), 0.05)
                if max_rt is not None:
                    sleep_s = min(sleep_s, max(0.01, float(max_rt) - (now - wait_started)))
                time.sleep(sleep_s)
        else:
    ...
    if args.resident or getattr(args, "background", False):
        # Keep adoption and the first operational heartbeat in one Python
        # process. Background mode then stays in the watcher and only prints
        # when a task/control arrives or the 24h report pulse fires.
        # Resident (Kimi) prints identity_adopted then exits on the first
        # task/control or --max-runtime-s so the host can read stdout.
        if getattr(args, "background", False):
            emit(
                {
                    "event": "identity_adopted",
                    "identity_file": str(identity_path),
                    "lwar_id": identity["lwar_id"],
                    "action": "background_watcher_running",
                }
            )
        else:
            emit(
                {
                    ...
                    "identity_file": str(identity_path),
                    "lwar_id": identity["lwar_id"],
                    "action": "resident_watcher_until_event_or_max_runtime",
                }
            )
        ns = _watch_namespace(identity_path, root, args)
        if max_rt is not None:
            remaining = float(max_rt) - (time.monotonic() - wait_started)
            if remaining <= 0:
                emit(
                    {
                        "event": "idle_timeout",
                ...
                return 10
            ns.max_runtime_s = remaining
        return watch(ns)
    emit({"event": "identity_adopted", "identity_file": str(identity_path), **identity})
    return 0


def command_adp_stop(args: argparse.Namespace) -> int:
    from .detach import pending_pid_path, stop_record, watcher_pid_path

    if args.pid_file:
        emit(stop_record(Path(args.pid_file)))
        return 0
    if args.identity_file:
        identity_path = Path(args.identity_file).resolve()
        identity = _load_or_exit(identity_path, "identity")
        root = resolve_identity_root(identity, identity_path, args.root)
        emit(stop_record(watcher_pid_path(root, identity["lwar_id"])))
        return 0
    if args.request_id:
        root = resolve_root(args.root)
        emit(stop_record(pending_pid_path(root, args.request_id)))
        return 0
    raise SystemExit("adp-stop requires --identity-file, --request-id, or --pid-file")


def command_adp_wait(args: argparse.Namespace) -> int:
    from .detach import event_log_path, wait_event_line
    ...
    if args.event_file:
        ...
        identity = _load_or_exit(identity_path, "identity")
        root = resolve_identity_root(identity, identity_path, args.root)
        event_file = event_log_path(root, identity["lwar_id"])
    elif args.request_id:
        root = resolve_root(args.root)
        event_file = event_log_path(root, args.request_id)
    else:
        raise SystemExit("adp-wait requires --event-file, --identity-file, or --request-id")
    payload = wait_event_line(event_file, timeout_s=args.timeout)
    emit(payload)
    return 0 if payload.get("event") != "idle_timeout" else 10


ADP_MAX_RUNTIME_S = 3000.0
KIMI_MAX_RUNTIME_S = ADP_MAX_RUNTIME_S


def _require_identity_file(args: argparse.Namespace) -> str:
    raw = args.identity_file or os.environ.get("PAO_LWAR_IDENTITY")
    if not raw:
        raise SystemExit("adp requires --identity-file or PAO_LWAR_IDENTITY")
    return raw


def command_adp_kimi(args: argparse.Namespace) -> int:
    """exit-notify ADP: resident watcher, exit on event or --max-runtime-s."""
    return run_exit_notify(
        _require_identity_file(args),
        root=args.root,
        interval=args.interval,
        timeout=args.timeout,
        lease_seconds=args.lease_seconds,
        max_runtime_s=args.max_runtime_s if args.max_runtime_s is not None else KIMI_MAX_RUNTIME_S,
    )


def command_adp_live(args: argparse.Namespace) -> int:
    """live-notify ADP: background watcher, emit each event on live stdout."""
...
def command_begin(args: argparse.Namespace) -> int:
    ...
    work = transport._mailbox(identity["lwar_id"]) / "work" / task_id
    work.mkdir(parents=True, exist_ok=True)
    draft_path = work / "result.json"
    ...
    emit(
        {
            ...
            "recovered": bool(grant.get("recovered")),
            "result_draft_path": str(draft_path),
...
def build_parser() -> argparse.ArgumentParser:
    ...
    response.add_argument(
        ...
        action="store_true",
        help=(
            "Kimi official / blocking: wait for OA approval, adopt, then exit "
            "on the first task/control/fatal or --max-runtime-s (use 3000)"
        ),
    )
    response.add_argument(
        "--background",
        action="store_true",
        help=(
            "normal ADP: wait for OA approval in-process, adopt, then run the "
            "background watcher (JSON line only on events or --report-every)"
        ),
    )
    ...
    response.add_argument(
        ...
        type=float,
        default=86400.0,
        help="with --background: watcher_report period in seconds (default 86400)",
    )
    response.add_argument("--state-wait-backoff-max", type=float, default=None)
    response.add_argument(
        "--detach",
        action="store_true",
        help="with --background: spawn detached child and return immediately",
    )
    response.add_argument(
        "--max-runtime-s",
        type=float,
        default=None,
        help="exit after this many seconds (official ADP: 3000) so the host can restart",
    )
    response.set_defaults(handler=command_response)

    def _add_adp_parser(name: str, *, help_text: str) -> None:
        p = subparsers.add_parser(name, help=help_text)
        p.add_argument("--identity-file")
        p.add_argument("--root", default=None)
        p.add_argument("--interval", type=float, default=5.0)
        p.add_argument("--timeout", type=float, default=90.0)
        p.add_argument("--lease-seconds", type=int, default=180)
        p.add_argument(
            "--max-runtime-s",
            type=float,
            default=ADP_MAX_RUNTIME_S,
            help="exit with idle_timeout after this many seconds (default 3000 = 50m)",
        )
        p.set_defaults(handler=command_adp_kimi)

    _add_adp_parser(
        "adp",
        help_text="exit-notify ADP (default): resident watcher; exit on event or 50 minutes",
    )
    _add_adp_parser(
        "adp-exit",
        help_text="alias of adp (exit-notify watcher)",
    )
    _add_adp_parser(
        "adp-kimi",
        help_text="alias of adp (kept for existing Kimi sessions)",
    )
    adp_live = subparsers.add_parser(
        "adp-live",
        help="live-notify ADP: stay up and emit each event on stdout",
    )
    adp_live.add_argument("--identity-file")
    adp_live.add_argument("--root", default=None)
    adp_live.add_argument("--interval", type=float, default=5.0)
    adp_live.add_argument("--timeout", type=float, default=90.0)
    adp_live.add_argument("--lease-seconds", type=int, default=180)
    adp_live.set_defaults(handler=command_adp_live)

    adp_stop = subparsers.add_parser("adp-stop")
    adp_stop.add_argument("--identity-file")
    adp_stop.add_argument("--request-id")
    adp_stop.add_argument("--pid-file")
    adp_stop.add_argument("--root", default=None)
    adp_stop.set_defaults(handler=command_adp_stop)

    adp_wait = subparsers.add_parser("adp-wait")
    adp_wait.add_argument("--event-file")
    adp_wait.add_argument("--identity-file")
    adp_wait.add_argument("--request-id")
    adp_wait.add_argument("--root", default=None)
    adp_wait.add_argument(
        "--timeout",
        type=float,
        default=3300.0,
        help="block until the next task/control event (default 3300s = 55m)",
    )
    adp_wait.set_defaults(handler=command_adp_wait)
