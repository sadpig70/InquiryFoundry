...
EXIT_NOTIFY_MAX_RUNTIME_S = 3000.0


def exit_notify_namespace(
    identity_file: str,
    *,
    root: str | None = None,
    interval: float = 5.0,
    ...
    """Watcher that exits on the first event or after 50 idle minutes."""
    return argparse.Namespace(
        identity_file=str(Path(identity_file).resolve()),
        root=root,
        interval=interval,
        timeout=timeout,
        ...
        detach=False,
        stop=False,
        max_runtime_s=EXIT_NOTIFY_MAX_RUNTIME_S if max_runtime_s is None else max_runtime_s,
    )


def live_notify_namespace(
    identity_file: str,
    *,
    root: str | None = None,
    interval: float = 5.0,
    ...
    """Watcher that stays up and emits each event on live stdout."""
    return argparse.Namespace(
        identity_file=str(Path(identity_file).resolve()),
        root=root,
        interval=interval,
