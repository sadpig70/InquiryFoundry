...
class FileTransport:
    ...
    def list_cancel_tombstones(self, lwar_id: str) -> list[Path]:
        folder = self._mailbox(lwar_id) / "cancelled"
        if not folder.is_dir():
            return []
        return sorted(folder.glob("*.json"))

    def finalize_cancel_tombstone(
        self, lwar_id: str, task_id: str, identity: dict[str, Any] | None = None
    ) -> str | None:
        """Close a tombstoned task if it is still pending. Returns action or None."""
        tomb = self._cancel_tombstone(lwar_id, task_id)
        if not tomb.is_file():
            return None
        if self.result_exists(lwar_id, task_id):
            tomb.unlink(missing_ok=True)
            return "tombstone_consumed_result_exists"
        pending = self.find_pending_task(lwar_id, task_id)
        if pending is None:
            return "tombstone_pending_absent"
        path, task = pending
        if path.parent.name == "incoming":
            dest = self._mailbox(lwar_id) / "claimed" / path.name
            if not claim_file(path, dest):
                return None
            path = dest
            task = safe_load_json(path) or task
            if not task.get("claim_token"):
                task["claim_token"] = new_id("claim")
                task["execution_id"] = task.get("execution_id") or new_id("execution")
                validate_contract(task, "task.schema.json")
        ...
        ident = identity or {
            ...
            "registry_version": task.get("registry_version", 1),
        }
        self._auto_cancel(ident, path, task)
        return "cancelled"
