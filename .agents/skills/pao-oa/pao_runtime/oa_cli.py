    ...
    for lwar_id in targets:
        ...
        if slot:
            ident = {
                ...
                "registry_version": slot.get("registry_version") or 1,
            }
        for tomb in transport.list_cancel_tombstones(lwar_id):
            task_id = tomb.stem
            action = transport.finalize_cancel_tombstone(lwar_id, task_id, ident)
            if action == "tombstone_pending_absent":
                entry = ledger.get(task_id)
                if entry and entry.get("status") in {"publishing", "published", "requeued"}:
                    contract = entry.get("task_contract") or {}
                    result = {
                        "schema_version": "pao.result.v1",
                        "task_id": task_id,
                        "workflow_id": entry["workflow_id"],
                        "lwar_id": lwar_id,
                        "instance_id": entry.get("instance_id") or contract.get("instance_id"),
                        "generation": entry.get("generation") or contract.get("generation"),
                        "registry_version": int(contract.get("registry_version") or 1),
                        "status": "cancelled",
                        "summary": "cancelled by tombstone; no pending mailbox claim",
                        "evidence": {
                            "cancelled_by": "oa_tombstone_reconcile",
                            "tombstone": f"cancelled/{task_id}.json",
                        },
                        "artifacts": [],
                        ...
                        "claim_token": contract.get("claim_token") or ("claim-" + ("0" * 32)),
                        "submitted_at": utc_now(),
                    }
                    outgoing = transport._mailbox(lwar_id) / "outgoing" / f"{task_id}.result.json"
                    outgoing.parent.mkdir(parents=True, exist_ok=True)
                    atomic_write_json(outgoing, result)
                    ledger.record_completed_result(result, str(outgoing))
                    tomb.unlink(missing_ok=True)
                    action = "ledger_cancelled"
            if action:
                closed.append({"lwar_id": lwar_id, "task_id": task_id, "action": action})
    return closed
...
def command_collect(args: argparse.Namespace) -> int:
    ...
    quarantined = []
    archived_reconciled = []
    tombstones_closed = _close_cancel_tombstones(transport, ledger, registry, targets)
    ...
    audit.record(
        ...
        "oa",
        {
            ...
            "tombstones_closed": len(tombstones_closed),
        },
    )
    emit(
        {
            ...
            "tombstones_closed": tombstones_closed,
        }
    )
    return 0
...
def command_recover(args: argparse.Namespace) -> int:
    ...
    registry = RegistryService(root).load_registry()
    _close_cancel_tombstones(transport, ledger, registry, targets)
