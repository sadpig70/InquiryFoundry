...
def command_adp_kimi(args: argparse.Namespace) -> int:
    """Official ADP: resident watcher, exit on event or --max-runtime-s, then restart."""
    ...
    identity_path = Path(raw).resolve()
    identity = _load_or_exit(identity_path, "identity")
    root = resolve_identity_root(identity, identity_path, args.root)
    max_runtime_s = args.max_runtime_s
    if max_runtime_s is None:
        max_runtime_s = KIMI_MAX_RUNTIME_S
...
def build_parser() -> argparse.ArgumentParser:
    ...
    _add_adp_parser(
        "adp",
        help_text="official ADP: resident watcher; exit on event or 50 minutes",
    )
    _add_adp_parser(
        "adp-kimi",
