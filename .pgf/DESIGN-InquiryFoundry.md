...
| D32 | **표적 복원은 세는 규칙이고 결함 성격이 적용 범위를 가른다.** 창은 조항 투입 런 이후 **모든** 마감 런, 문턱은 **한 런 2건** 또는 **누적 3건**(리셋 없음). **형태 결함**(생성기가 자기 산출물로 자가 검사 가능)에만 배급을 복원하고, **포트폴리오 결함**(가시성 감옥 밖 정보를 알아야 피할 수 있음 — `DUP-RESUBMIT`)에는 복원이 무력하므로 OA 측 사전 선별 검토를 사람 안건으로 상정한다. 성격은 조항 작성 시점에 `defect_kind` 로 한 번 기록한다. |
