...
| D31 | **리뷰어의 분류 어휘는 휴면으로 깎이지 않는다.** 불용 퇴출은 생성기 프롬프트 비용을 위한 것이었고 D27 이 그 비용을 없앴다. 남은 효과는 리뷰어 `known_patterns` 를 깎는 것뿐인데, 깎이는 것은 **조항이 성공한 코드**다 — 결함이 사라졌다는 사실이 코드를 은퇴시킨다. 이름을 못 찾은 리뷰어는 `NEW` 를 써서 기존 코드에 두 번째 이름을 붙이고(§7.18 비수렴), **재발 카운트가 경보를 울려야 할 런에서 0 으로 끊긴다.** 휴면은 "최근 관측 없음" **표시**로 강등한다. `dormancy_display_only.ratified` 로 사람 비준 후 발효 — **2026-08-25 비준됨**. 어휘 압축은 상한 16 과 사람의 병합뿐이며 코드 13개에서 자동으로 안건이 된다. |
| D32 | **표적 복원은 세는 규칙이고 결함 성격이 적용 범위를 가른다.** 창은 **조항이 브리프에 실린 상태로 마감된 모든 런 — 투입 런 포함**(2026-08-27 Fable 재확정: "이후"가 두 독해를 낳아 live23 의 경계 사례에서 갈렸다. 효력은 생성기가 조항을 받은 순간 시작한다). 문턱은 **한 런 2건** 또는 **누적 3건**(리셋 없음). **형태 결함**(생성기가 자기 산출물로 자가 검사 가능)에만 배급을 복원하고, **포트폴리오 결함**(가시성 감옥 밖 정보를 알아야 피할 수 있음 — `DUP-RESUBMIT`)에는 복원이 무력하므로 OA 측 사전 선별 검토를 사람 안건으로 상정한다. 성격은 조항 작성 시점에 `defect_kind` 로 한 번 기록한다. |
