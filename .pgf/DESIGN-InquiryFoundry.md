...
| D31 | **리뷰어의 분류 어휘는 휴면으로 깎이지 않는다.** 불용 퇴출은 생성기 프롬프트 비용을 위한 것이었고 D27 이 그 비용을 없앴다. 남은 효과는 리뷰어 `known_patterns` 를 깎는 것뿐인데, 깎이는 것은 **조항이 성공한 코드**다 — 결함이 사라졌다는 사실이 코드를 은퇴시킨다. 이름을 못 찾은 리뷰어는 `NEW` 를 써서 기존 코드에 두 번째 이름을 붙이고(§7.18 비수렴), **재발 카운트가 경보를 울려야 할 런에서 0 으로 끊긴다.** 휴면은 "최근 관측 없음" **표시**로 강등한다. `dormancy_display_only.ratified` 로 사람 비준 후 발효. 어휘 압축은 상한 16 과 사람의 병합뿐이며 코드 13개에서 자동으로 안건이 된다. |
