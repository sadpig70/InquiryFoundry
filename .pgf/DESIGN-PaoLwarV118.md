...
검증되지 **않은** 것(당시): 이종 벤더 동시 3-LWAR, 50분 슬라이스 경계, cancel/drain 경로, 축소 `slice_s`의 실제 동작.

### 추가 검증 — 50분 슬라이스 경계 (2026-08-17 관측)

LWAR3가 `2026-08-16T22:13:37Z` ~ `2026-08-17T23:24:46Z` 동안 **`invocation_started` 31회**를 기록했다.
간격은 3013~3075초(중앙값 ≈ 3017초)로 `--max-runtime-s 3000` + 재시작 오버헤드와 일치하며,
...
## 19. 3-LWAR 동시 가동과 `--auto` 라우팅 (2026-08-18)

Qwen 세션 재개로 **이종 벤더 3대가 동시에 `active`** 가 됐다 — HANDOFF §7-1의 원래 목표다.

### 19.1 RESUME 분기 — 재등록 없는 복귀

Qwen 재개는 §0.5의 `status --identity-file` → **exit 0 → RESUME** 경로였다. 이 세션에서 처음 실행됐다
(그 전 등록은 전부 fresh register였다).

| 확인 | 결과 |
|---|---|
| 재등록 요청 | **0건** |
| `registry_version` | **20 불변** (레지스트리 변이 없음) |
| generation | **3 유지** (재등록이었다면 4) |
| 슬라이스 | `--max-runtime-s 540` 그대로 재도출 |

P5의 exit code 사전에서 *"exit 0 = RESUME, 절대 재등록 금지"* 로 정리한 분기가 그대로 동작했다.
슬라이스가 세션 간에 재현된다는 것도 확인됐다 — 프로브 판정이 호스트 속성에서 결정론적으로 나온다는 뜻이다.

### 19.2 `--auto` 라우팅 — 3-way 경합

`--auto --require-capability coding` 으로 3연속 발행:

| 발행 | 라우팅 | 근거 |
|---|---|---|
| `auto1` | **LWAR1** | 백로그 0 동점 → 번호 최소 |
| `auto2` | **LWAR2** | LWAR1 백로그 1 → 다음 최소 |
| `auto3` | **LWAR3** | 남은 최소 |

`publish.md`의 *"동점은 백로그 최소, 그 다음 LWAR 번호 최소"* 가 3-way에서 성립.
3개 벤더에 정확히 1건씩 분산됐고 병렬 실행·수집이 간섭 없이 끝났다(`workflow-status` → `completed` 3/3).

### 19.3 벤더 간 직접 비교 — 같은 형태, 다른 계약 크기

세 태스크의 criteria 수를 **4 / 5 / 6** 으로 달리해 `count.txt` 정답이 서로 다르게 했다.
지시문은 동일하며, ack4에서 통과를 이끌어낸 명시적 표현(*"배열을 열거하라"*, *"길이가 아니라 전체 바이트 시퀀스"*)을 썼다 —
`publish.md`에 넣은 작성 지침을 OA 자신에게 적용한 것이다.

**3건 전부 통과.** OA가 바이트를 재검증했고 criteria 충족을 개별 판정했다.

| 슬롯 | 벤더 | 관측된 성향 |
|---|---|---|
| LWAR1 | alibaba | evidence가 가장 두껍다 — 파일별 `sha256`, `byte_contents`, **독립 2회 검증**. 요구되지 않은 `completion_criteria_count`까지 자발적으로 넣고 값도 맞았다 |
| LWAR2 | moonshot | 요구된 것을 정확히, 최소로. ack3 실패 이후 **3연속 통과** |
| LWAR3 | xai | 전 criteria 충족. 다만 바이트 수를 `byte_counts` 맵이 아닌 `ack_txt_bytes` 등 개별 키로 냈고, `commands`는 헬퍼 스크립트 경로만 적었으며, cwd에 헬퍼 `_write_exact.py`를 남겼다(어느 criterion·지시도 금지하지 않음) |

판정 시 **키 이름을 요구하지 않은 criterion에 키 이름을 요구하지 않았다** — criterion 4는 "바이트 수를 나열하라"이지
"`byte_counts` 맵으로 내라"가 아니다. 형식이 아니라 요구된 정보의 존재로 판정하는 것이 정확하다.

이 성향 차이는 향후 라우팅 판단의 근거가 된다. 정밀 검증이 필요한 태스크와 단순 실행 태스크를 다른 슬롯에 보낼 수 있다.
